...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	/**
	 * Spring容器初始化的核心方法,运用到父类模板设计模式(根据不同的上下文对象,调用不同的子类方法)
	 * Spring核心上下文对象有:
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * 1. 创建BeanFactory对象
			 * 2. 自定义标签解析
			 * 	2.1 传统标签解析<bean/>,<import/>等
			 * 	2.2 自定义标签解析 <context:component-scan base-package="com.XX.XX/>
			 * 	2.3 自定义标签解析流程
			 * 		2.3.1 根据标签的命名空间找到对应的namespaceUri
			 * 		2.3.2 加在对应jar包中的spring.handlers文件,建立映射关系
			 * 		2.3.3 根据namespaceUri从映射关系中找到对应的实现了NamespaceHandler接口的类
			 * 		2.3.4 调用类的init方法,init方法是注册了各种自定义标签的解析类
			 * 		2.3.5 根据namespaceUri找到对应的解析类,然后调用parser方法完成标签解析
			...
			try {
				...
				//实例化并调用所有已注册的 BeanFactoryPostProcessor bean，如果给定，则遵从显式顺序。
				//必须在单例实例化之前调用
				//是对BeanDefinitionRegistryPostProcessor和BeanFactoryPostProcessor两个接口的调用
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				// 把实现了BeanPostProcessor接口的类实例化,交给BeanFactory管理
	...
	protected void prepareRefresh() {
		...
		//初始化上下文环境中加载的其他资源文件(jdbc.properties),默认什么也不加载
		initPropertySources();

		// Validate that all properties marked as required are resolvable:
		// see ConfigurablePropertyResolver#setRequiredProperties
		//校验所有标记为required的属性都是可以解析的(通过ConfigurablePropertyResolver#setRequiredProperties方法标记)
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//beanFactory为DefaultListableBeanFactory;getBeanFactoryPostProcessors()如果没有自定义的BeanFactoryPostProcessor,这里默认为null
	...
	protected void initApplicationEventMulticaster() {
		//获取容器对象->DefaultLisableBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		//判断容器中是否有一个叫applicationEventMulticaster的Bean，默认是没有的
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			...
			//没有就创建SimpleApplicationEventMulticaster对象作为事件多播器
	...
	protected void registerListeners() {
		...
		//这个阶段监听器没有初始化,因为要让BeanPostProcessors感应到
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置类型转换器
		// 最先初始化名字为 conversionService的类,它用来将前端传过来的参数和后端的 controller 方法上的参数进行绑定的时候用
